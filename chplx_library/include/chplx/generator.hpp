...
using rebind = typename std::allocator_traits<Allocator>::template rebind_alloc<
    aligned_block>;
...
template <typename Alloc = void> class promise_allocator {
  ...
  static char *allocate(Allocator alloc, std::size_t const size) {
    if constexpr (std::is_default_constructible_v<Allocator> &&
      ...
      // do not store stateless allocator, but store size of
      //   block
      std::size_t const count =
          (size + sizeof(aligned_block) + sizeof(std::size_t) - 1) /
      ...
      *reinterpret_cast<std::size_t *>(address) = size;
      ...
      std::size_t const count =
          (size + sizeof(Allocator) + sizeof(std::size_t) + align - 1) /
          sizeof(aligned_block);
      ...
      *reinterpret_cast<std::size_t *>(address) = size;
      ...
      auto const alloc_address =
          (reinterpret_cast<std::uintptr_t>(ptr) + size + sizeof(std::size_t) +
           alignof(Allocator) - 1) &
          ~(alignof(Allocator) - 1);
  ...
  static void dealloc(void *const ptr, std::size_t const size) {
    if constexpr (std::is_default_constructible_v<Allocator> &&
      ...
      Allocator alloc{};
      std::size_t const count =
          (size + sizeof(std::size_t) + sizeof(aligned_block) - 1) /
          sizeof(aligned_block);
      ...
      auto const address =
          (reinterpret_cast<std::uintptr_t>(ptr) + sizeof(std::size_t) + size +
           alignof(Allocator) - 1) &
          ~(alignof(Allocator) - 1);

      auto &stored_allocator = *reinterpret_cast<Allocator *>(address);
      ...
      static constexpr std::size_t align =
          (std::max)(alignof(Allocator), sizeof(aligned_block));
      std::size_t const count =
          (size + sizeof(std::size_t) + sizeof(Allocator) + align - 1) /
          sizeof(aligned_block);
...
public:
  ...
  // some older versions of gcc complain about
  //   mismatched - new -
  //       delete
...
template <> class promise_allocator<void> {
  ...
  static char *allocate(ProtoAlloc const &proto, std::size_t size) {
    ...
    if constexpr (std::is_default_constructible_v<Allocator> &&
      ...
      // allocated block
      dealloc_fn const dealloc = [](void *const ptr, std::size_t const s) {
        Allocator alloc{};
        std::size_t const count =
      ...
      *reinterpret_cast<std::size_t *>(address) = size;
      ...
      dealloc_fn const dealloc = [](void *const ptr, std::size_t s) {
        s += sizeof(std::size_t) + sizeof(dealloc_fn);
        auto const address = (reinterpret_cast<std::uintptr_t>(ptr) + s +
                              alignof(Allocator) - 1) &
                             ~(alignof(Allocator) - 1);

        auto &stored_allocator = *reinterpret_cast<Allocator const *>(address);
        Allocator alloc{HPX_MOVE(stored_allocator)};
        stored_allocator.~Allocator();

        std::size_t const count =
            (s + sizeof(alloc) + align - 1) / sizeof(aligned_block);

        alloc.deallocate(static_cast<aligned_block *>(ptr), count);
      };

      std::size_t const count = (size + sizeof(dealloc_fn) + sizeof(alloc) +
                                 align + sizeof(std::size_t) - 1) /
                                sizeof(aligned_block);
      ...
      *reinterpret_cast<std::size_t *>(address) = size;
      ...
      auto const alloc_address =
          (reinterpret_cast<uintptr_t>(ptr) + size + alignof(Allocator) - 1) &
          ~(alignof(Allocator) - 1);
...
public:
  ...
  void *operator new(std::size_t const size) {
    void *const ptr =
        ::operator new[](size + sizeof(std::size_t) + sizeof(dealloc_fn));
  ...
  void *operator new(size_t const size, This const &, std::allocator_arg_t,
                     Alloc const &alloc, Args const &...) {
  ...
  void operator delete(void *const ptr) noexcept {
    // the size is stored before the address
    char *address = static_cast<char *>(ptr) - sizeof(std::size_t);
    std::size_t const size = *reinterpret_cast<std::size_t *>(address);

    dealloc_fn dealloc;
    std::memcpy(&dealloc, static_cast<char *>(ptr) + size, sizeof(dealloc_fn));
    dealloc(address, size);
  }
...
template <typename Ref, typename V>
using gen_reference_t = std::conditional_t<std::is_void_v<V>, Ref &&, Ref>;

template <typename Ref>
using gen_yield_t =
    std::conditional_t<std::is_reference_v<Ref>, Ref, Ref const &>;

template <typename Value, typename Ref> class gen_iter;

template <typename Yielded> class gen_promise_base {
public:
  static_assert(std::is_reference_v<Yielded>);

  static constexpr hpx::suspend_always initial_suspend() noexcept { return {}; }

  static constexpr auto final_suspend() noexcept { return final_awaiter{}; }
  ...
  // clang-format off
            auto
            yield_value(std::remove_reference_t<Yielded> const& val)
            noexcept(
...
private:
  struct element_awaiter {
    ...
    constexpr void
    await_suspend(hpx::coroutine_handle<Promise> handle) noexcept {
#ifdef __cpp_lib_is_pointer_interconvertible
      static_assert(std::is_pointer_interconvertible_base_of_v<gen_promise_base,
                                                               Promise>);
  ...
  struct final_awaiter {
    ...
    await_suspend(hpx::coroutine_handle<Promise> handle) noexcept {
      ...
      hpx::coroutine_handle<gen_promise_base> cont = current.info->parent;
  ...
  template <typename Ref, typename V, typename Alloc> struct nested_awaitable {
    static_assert(std::same_as<gen_yield_t<gen_reference_t<Ref, V>>, Yielded>);

    nest_info nested;
    generator<Ref, V, Alloc> gen;

    explicit nested_awaitable(generator<Ref, V, Alloc> &&gen_) noexcept
        : gen(HPX_MOVE(gen_)) {}

    constexpr bool await_ready() noexcept { return !gen.coro; }

    template <typename Promise>
    hpx::coroutine_handle<gen_promise_base>
    await_suspend(hpx::coroutine_handle<Promise> current) noexcept {
      ...
      auto target = hpx::coroutine_handle<gen_promise_base>::from_address(
          gen._Coro.address());
      nested.parent = hpx::coroutine_handle<gen_promise_base>::from_address(
          current.address());
  ...
  template <typename, typename> friend class gen_iter;

  // top and info are mutually exclusive, and could potentially be
  //   merged.
...
public:
  ...
  gen_iter(gen_iter &&that) noexcept : coro{::std::exchange(that.coro, {})} {}
  ...
  Ref operator*() const noexcept {
    // HPX_ASSERT_MSG(
    //     !coro.done(), "Can't dereference generator end
    //     iterator");
    return static_cast<Ref>(*coro.promise().top.promise().ptr);
  }

  gen_iter &operator++() {
    HPX_ASSERT_MSG(!coro.done(), "Can't increment generator end iterator");
  ...
  [[nodiscard]] friend bool operator==(gen_iter lhs,
                                       hpx::default_sentinel_t) noexcept {
    return lhs.coro.done();
  }
  [[nodiscard]] friend bool operator!=(gen_iter lhs,
                                       hpx::default_sentinel_t rhs) noexcept {
    return !(lhs == rhs);
  }

  [[nodiscard]] friend bool operator==(hpx::default_sentinel_t,
                                       gen_iter rhs) noexcept {
    return rhs.coro.done();
  }
  [[nodiscard]] friend bool operator!=(hpx::default_sentinel_t lhs,
                                       gen_iter rhs) noexcept {
    return !(lhs == rhs);
  }

private:
  template <typename, typename, typename> friend struct hpx::generator;

  explicit gen_iter(
      gen_secret_tag,
      hpx::coroutine_handle<gen_promise_base<gen_yield_t<Ref>>> coro_) noexcept
      : coro{coro_} {}
...
template <typename Ref, typename V, typename Allocator> struct generator {
private:
  ...
  static_assert(
      detail::common_reference_with<reference &&, value &> &&
...
public:
  ...
  generator(generator &&that) noexcept : coro(::std::exchange(that.coro, {})) {}
  ...
  [[nodiscard]] detail::gen_iter<value, reference> begin() const {
    ...
    // clang-format off
            return detail::gen_iter<value,
            reference>{detail::gen_secret_tag{},
