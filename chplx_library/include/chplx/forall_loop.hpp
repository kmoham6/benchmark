...
// forall loop for ranges
template <typename Policy, typename T, BoundedRangeType BoundedType,
          bool Stridable, typename F, typename... Args>
void forall(Policy &&p, Range<T, BoundedType, Stridable> const &r, F &&f,
            Args &&...args) {
  // if (chplx_fork_join_executor) {
  //   HPX_ASSERT(exec != nullptr);
  //   hpx::ranges::experimental::for_loop(
  //       std::forward<Policy &&>(p).on(*exec), detail::IteratorGenerator(r),
  //       [&, ... fargs = detail::task_intent<std::decay_t<Args>>::call(
  //               std::forward<Args>(args))]<typename Arg>(Arg &&value) mutable
  //               {
  //         f(std::forward<Arg>(value),
  ...
  // } else {
  hpx::ranges::experimental::for_loop(
      std::forward<Policy &&>(p), detail::IteratorGenerator(r),
      [&, ... fargs = detail::task_intent<std::decay_t<Args>>::call(
              std::forward<Args>(args))]<typename Arg>(Arg &&value) mutable {
        f(std::forward<Arg>(value),
          hpx::util::decay_unwrap<decltype(fargs)>::call(fargs)...);
...
template <typename Policy, typename T, typename F, typename... Args>
void forall(Policy &&p, Range<T, BoundedRangeType::bounded, false> const &r,
            F &&f, Args &&...args) {
  // if (chplx_fork_join_executor) {
  //   HPX_ASSERT(exec != nullptr);
  //   hpx::ranges::experimental::for_loop(
  //       std::forward<Policy &&>(p).on(*exec), r.these(),
  //       [&, ... fargs = detail::task_intent<std::decay_t<Args>>::call(
  //               std::forward<Args>(args))]<typename Arg>(Arg &&value) mutable
  //               {
  //         f(*std::forward<Arg>(value),
