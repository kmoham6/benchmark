...
void measureAdjacent_differenceAlgorithms() {
  ...
  std::ofstream fout(
      "/work/karame.mp/hpx_June/hpx_buran/performance/result.csv",
      std::ios_base::app);
  ...
  for (size_t s = start; s <= till; s *= 2) {
    ...
    for (int i = 0; i <= NUM_ITERATIONS + 5; i++) {
      ...
      // chplx::forall(hpx::execution::seq, chplx::Range{1, NX - 1}, [&](auto i)
      // {
      //   int y = i;
      //   int x = i;
      //   auto abc_res =
      //       std::pow(
      //           std::sin(std::tan(std::pow(x, 3)) * std::cos(std::pow(y,
      //           3))), 5) +
      //       std::pow(
      //           std::cos(std::tan(std::pow(y, 3)) * std::sin(std::pow(x,
      //           3))), 5) +
      //       std::exp(std::sqrt(
      ...
      //       std::pow(std::hypot(std::log(std::abs(x)),
      //       std::log(std::abs(y))),
      //                4) *
    ...
    for (int i = 0; i <= NUM_ITERATIONS + 5; i++) {
      ...
      constexpr int NX = 1000;
      chplx::forall(
          hpx::execution::par.with(nc, scs), chplx::Range{1, NX - 1},
          [&](auto i) {
            int y = i;
            int x = i;
            auto abc_res =
