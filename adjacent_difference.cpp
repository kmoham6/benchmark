...
double test(ExPolicy policy, std::size_t n)
{
    ...
    hpx::execution::adaptive_core_chunk_size scs (n/2);
    ...
    {
        hpx::generate(hpx::execution::par.with(scs), nums.begin(), nums.end(), gen_float_t{});
    }
    else
    {
    ...
    if constexpr (hpx::is_parallel_execution_policy_v<ExPolicy>)
        hpx::adjacent_difference(policy.on(executor).with(scs), nums.begin(), nums.end(),
                                 nums2.begin());
    else
...
int hpx_main()
{
    ...
    for (std::size_t i = 10; i <= 30; i++)
    {
        ...
        double seq = test3(hpx::execution::seq, 10, n);
        double par = test3(hpx::execution::par, 10, n);
        std::cout << "n : " << i << "\t";
        std::cout << "seq : " << seq << "\t";
        std::cout << "par : " << par << "\t";
